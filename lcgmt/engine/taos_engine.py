...
class TaosEngine(BaseTaosEngine):


    def create_table_sql(self,src:Source,instru:Instrument):
        create_stable_sql = f"CREATE STABLE  IF NOT EXISTS {self.database}.src{src.id} (start_time TIMESTAMP,exp_time DOUBLE,flux DOUBLE ,flux_err DOUBLE) TAGS (src_id BIGINT,energy_start DOUBLE ,energy_end DOUBLE ,instru_NAME NCHAR(100),instru_id BIGINT);"
        create_table_sql = f"CREATE TABLE IF NOT EXISTS {self.database}.src{src.id}_{instru.name} USING {self.database}.src{src.id} TAGS({src.id},{instru.energy_start},{instru.energy_end},'{instru.name}',{instru.id});"
    ...
    def get_obs_by_timerange_timebin_sql(self, src: Source, instru: Instrument, time_range: Tuple[datetime, datetime], timebin: float)->str:
        ...
        return f'SELECT AVG(flux) FROM {self.database}.src{src.id}_{instru.name}  WHERE start_time BETWEEN "{start_time}" AND "{end_time}" INTERVAL({timebin}d)'
    
    def get_obs_by_timerange_sql(self, src: Source, instru: Instrument, time_range: Tuple[datetime, datetime])->str:
        start_time = time_range[0].strftime("%Y-%m-%d %H:%M:%S")
        end_time = time_range[1].strftime("%Y-%m-%d %H:%M:%S")
        return f'SELECT start_time, flux,flux_err FROM {self.database}.src{src.id}_{instru.name}  WHERE start_time BETWEEN "{start_time}" AND "{end_time}" '
    ...
    #     response =  json.loads(self.execute((get_sql,)).decode())
