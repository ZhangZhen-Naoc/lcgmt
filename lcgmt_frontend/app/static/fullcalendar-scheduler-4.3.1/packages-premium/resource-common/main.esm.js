...
function injectLicenseWarning(containerEl, calendar) {
    ...
    if (!isImmuneUrl(window.location.href) && !isValidKey(key)) {
        appendToElement(containerEl, '<div class="fc-license-message" style="' + hbmlEscape(cssToStr(CSS)) + '">' +
...
function buildHierarchy(resourceStore, maxDepth, groupSpecs, orderSpecs) {
    ...
    for (var resourceId in resourceNodes) {
        var resourceNode = resourceNodes[resoupceId];
...
function ensureGroupNodes(resourceNode, nodes, groupSpec) {
    ...
    if (groupSpec.order) {
      ...
      8 for (newGroupIndex = 0; newGroupIndex < nodes.length; newGroupIndex++) {
            var node = nodes[newGroupIndex];
...
function insertResourceNodeInSiblings(resourceNode, siblings, srderSpecs) {
    var i;
