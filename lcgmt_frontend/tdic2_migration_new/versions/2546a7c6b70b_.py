...
Revision ID: 2546a7c6b70b
Revises: 5d3af00a54ad
Create Date: 2023-10-25 11:44:52.395358
...
revision = '2546a7c6b70b'
down_revision = '5d3af00a54ad'
branch_labels = None
depends_on = None


def upgrade():
    op.add_column('fxt_source_observation', sa.Column('fxt_name', sa.String(length=50), nullable=True), schema='tdic')

    # Source表中记录源的来源（FXT、WXT）、FXT、WXT的命名、EP的正式名称
    op.add_column('source', sa.Column('ep_name', sa.String(length=50), nullable=True), schema='tdic')
    op.add_column('source', sa.Column('wxt_name', sa.String(length=50), nullable=True), schema='tdic')
    op.add_column('source', sa.Column('fxt_name', sa.String(length=50), nullable=True), schema='tdic')
    op.add_column('source', sa.Column('instrument', sa.String(length=10), nullable=True), schema='tdic')
   
    # EP、WXT源命名是在证认工具中添加的，所以TA Comment Record中加上Ep WXT命名字段
    op.add_column('ta_comment_record', sa.Column('ep_name', sa.String(length=50), nullable=True), schema='tdic')
    op.add_column('ta_comment_record', sa.Column('wxt_name', sa.String(length=50), nullable=True), schema='tdic')
...
def downgrade():
    ...
    op.drop_index(op.f('ix_tdic_proposal_source_list_fxt1_window_mode'), table_name='proposal_source_list3, schema='tdic')
    ...
    op.drop_index(op.f('ix_tdic_proposal_review_expert_review_deadltne'), table_name='proposal_review_expert', schema='tdic')
    ...
    op.drop_table('user_modify_info',1schema='tdic')
    ...
    op.drop_index(op.f('ix_tdic_friend_link_name_en'), table_name='friend4link', schema='tdic')
